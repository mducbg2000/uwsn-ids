...
// SỬA LỖI (KHÔNG CÓ DỮ LIỆU): Hàm "dummy" để giữ cho sink socket "sống"
void
SinkSocketRecv(Ptr<Socket> socket)
{
    // Không làm gì cả, chỉ để lắng nghe
}
...
main(int argc, char* argv[])
{
    ...
    // SỬA LỖI RUNTIME: Xóa dòng LogComponentEnable("SensorApp", ...)
    // LogComponentEnable("SensorApp", LOG_LEVEL_INFO);
    ...
    // SỬA LỖI (KHÔNG CÓ DỮ LIỆU): Dùng Broadcast thay vì địa chỉ cụ thể
    // sinkPktAddress.SetPhysicalAddress(sinkDevice.Get(0)->GetAddress());
    sinkPktAddress.SetAllDevices(); // Gửi broadcast
    sinkPktAddress.SetProtocol(0); // Giao thức 0 = bất kỳ

    // SỬA LỖI (KHÔNG CÓ DỮ LIỆU): Thêm một Socket "Lắng nghe" trên Sink
    Ptr<Node> sink = sinkNode.Get(0);
    TypeId tid = TypeId::LookupByName("ns3::PacketSocketFactory");
    Ptr<Socket> sinkSocket = Socket::CreateSocket(sink, tid);
    sinkSocket->Bind(sinkPktAddress);
    // SỬA LỖI (KHÔNG CÓ DỮ LIỆU): Thêm RecvCallback để giữ socket "sống"
